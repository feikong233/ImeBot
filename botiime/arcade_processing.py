...
def arcade_query_return(input_sx, area="arcade"):
    ...
    # 获取机厅数量jts
    cur.execute("SELECT COUNT(id) FROM " + area)
    jts = cur.fetchall()
    ...
    # 从数据库提取中文机厅缩写，加在列表里
    while num <= jts[0][0]:
        cur.execute("SELECT chn FROM " + area + " WHERE id=" + str(num))
        sx.append(cur.fetchall())
        num = num + 1
    ...
    for i in sx:
        if str(input_sx[0]) == str(i[0][0]):
            ...
            cur.execute("SELECT fullname FROM " + area + " WHERE name='" + str(input_sx[0]) + "' " + "OR chn='" + str(input_sx[0]) + "' ")
            fullname = str(cur.fetchall()[0][0])
            cur.execute("SELECT people FROM " + area + " WHERE name='" + str(input_sx[0]) + "' " + "OR chn='" + str(input_sx[0]) + "' ")
...
def people_query(area="arcade"):
    ...
    while num <= int(jts[0][0]):
        # 获取对应的机厅名称
        cur.execute("SELECT fullname FROM " + area + " WHERE id=" + str(num))
        jtmc = cur.fetchall()
        # 获取对应的机厅人数
        cur.execute("SELECT people FROM " + area + " WHERE id=" + str(num))
        jtrs = cur.fetchall()
        # 获取对应的机厅缩写
        cur.execute("SELECT name FROM " + area + " WHERE id=" + str(num))
        jtsx = cur.fetchall()
...
def changes_upload(n, area="arcade"):
    ...
    if re.match('[a-z]+[+-][0-9]+', n):
        ...
        for i in sx:
            if str(jtsx[0]) == str(i[0][0]):
                # 获取对应机厅的人数并存储
                cur.execute("SELECT people FROM " + area + " WHERE name='" + str(jtsx[0]) + "'")
    ...
    # 匹配汉字部分
        ...
        for i in sx:
            if str(jtsx[0]) == str(i[0][0]):
                # 获取对应机厅的人数并存储
                cur.execute("SELECT people FROM " + area + " WHERE name='" + str(jtsx[0]) + "' " + "OR chn='" + str(jtsx[0]) + "' ")
                rs_past = int(cur.fetchall()[0][0])
                # 判断正负
                if str(zj[0]) == '+':
                    rs_now = rs_past + int(jtrs[0])
